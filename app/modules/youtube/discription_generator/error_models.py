"""
Error models for Description Generator module
"""
...
class DescriptionErrorCodes:
    """Error codes for Description Generator module"""
    # Description Generation Errors
    DESCRIPTION_GENERATION_FAILED = "DESC_001"
    DESCRIPTION_SAVE_FAILED = "DESC_002"
    DESCRIPTION_REGENERATION_FAILED = "DESC_003"
    ...
    # Validation Errors
    INVALID_DESCRIPTION = "VAL_001"
    DESCRIPTION_TOO_LONG = "VAL_002"
    DESCRIPTION_TOO_SHORT = "VAL_003"
    INVALID_TEMPLATE = "VAL_004"
...
class DescriptionGenerationError(Exception):
    ...
    message: str = "Description generation failed"
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.DESCRIPTION_GENERATION_FAILED,
            details={
...
@dataclass
class DescriptionSaveError(Exception):
    """Raised when description save fails"""
    message: str = "Failed to save description"
    video_id: Optional[str] = None
    user_id: Optional[str] = None
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.DESCRIPTION_SAVE_FAILED,
            details={
                "video_id": self.video_id,
                "user_id": self.user_id,
                "error_type": self.error_type
...
class VideoNotFoundError(Exception):
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.VIDEO_NOT_FOUND,
...
class VideoTranscriptNotFoundError(Exception):
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.VIDEO_TRANSCRIPT_NOT_FOUND,
            details={"video_id": self.video_id, "user_id": self.user_id} if self.video_id or self.user_id else None
        )
        super().__init__(self.error_detail.message)
...
class ApiKeyMissingError(Exception):
    ...
    message: str = "API key is required for description generation"
    user_id: Optional[str] = None
    
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.API_KEY_MISSING,
...
class ValidationError(Exception):
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.INVALID_DESCRIPTION,
...
class DatabaseError(Exception):
    ...
    def __post_init__(self):
        self.error_detail = ErrorDetail(
            message=self.message,
            code=DescriptionErrorCodes.DATABASE_ERROR,
